...
from extra.shared_arrays import (
    ArrayQueue,
    IndexedArrayQueue,
...
class SourceProcess(Process):
    ...
    def run(self):
        ...
        for i in range(self.n_items):
            try:
                # self.source_array.put(5)
                self.source_array.put(np.full((100, 100), 5, np.uint8))
                print("I inserted item # ", self.source_array.view.idx_item)
    ...
    def run(self):
        while True:
            try:
                ...
                print(f"Got item: # {self.source_array.last_item}, {self.source_array.view.idx_item}")
                assert item[0, 0] == 5
            except Empty:
                break
